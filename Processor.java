...
public class Processor {
    ...
    public Processor() {
        pool = new LinkedList();
        this.migratingPool = new LinkedList();
    ...
    void addToMigratingPool(Job newProgram) {
        this.migratingPool.add(newProgram);
    ...
    boolean checkDeadline(long t) {
        if (executing.top != null) { //if something is in stack
            ...
            if (executing.top.timeleft == 0) {//if its doene 
                this.stretch = stretch + executing.top.Stretch(t); //get stretch
                this.stretchCount++;
...
//System.out.println("COMPLETED 1:");
                ...
                //System.out.println(tempReward);
                //System.out.println("");
                if (executing.top.getR() + executing.top.p >= t) { //if on time
                    numCompleted++;
                }
                executing.remove();
    ...
    boolean checkDeadlineEDFOS(long t) { //replace timeleft with Deadline
        if (executing.top != null) { //if something is in stack
            ...
            if (executing.top.timeleft < 1) {//if its doene

                this.stretch = stretch + executing.top.Stretch(t); //get stretch
                stretchCount++;
                double temp = executing.top.getReward(t);

                if (executing.top.deadline > -1) {
                    numCompleted++;
                }
