...
public class LBBA3 {
    ...
    LBBA3(int numProcessors2) //initializes the number of processors
    {
        preemptions = 0;
    ...
    int minProcessor() //finds the least-busy workload UNIQUE CHANGE FOR UTILIZATION
    {
        double minU = servers[0].getUtilization();
        int index = 0;

        for (int i = 1; i < numProcessors; i++) {
            double temp = servers[i].getUtilization();
            if (temp < minU) {
                minU = temp;
                index = i;
            }
    ...
    void event(long t) {
        ...
        else if (count == 0) //else no empty stack
        {
            ...
            while (TempList.front != null && i < numProcessors) {
                if (TempList.front.getD() > (4 * servers[i].executing.top.getD())) { //preempt now
        ...
        //sort TempList by utilization
        this.TempList.insertionSortUtilization(TempList.front, t);
        
         //decide which processor to push the tempList object onto, partition, based on utilization
 ...
 void findTotalReward() {
    ...
    public indexer[] makeIndexers() {
        ...
        for (int i = 0; i < this.numProcessors; i++) {
            indexers[i] = new indexer(this.servers[i].getUtilization(), i);
    ...
    public void quickSort(indexer[] arr, int low, int high) {
        ...
        double pivot = arr[middle].utilization;

        // make left < pivot and right > pivot
        int i = low, j = high;
        while (i <= j) {
            while (arr[i].utilization < pivot) {
                i++;
            }

            while (arr[j].utilization > pivot) {
                j--;
            }
    ...
    public class indexer {
        ...
        public indexer(double utilization2, int index2) {
            index = index2;
